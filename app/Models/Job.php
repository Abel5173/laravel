...
class Job extends Model{
    ...
    // protected $fillable = ['employer_id', 'title', 'salary']; // This is an array of the columns in the table that can be mass assigned. This is important because Laravel uses mass assignment to insert data into the database.
    protected $guarded = []; // This is an array of the columns in the table that cannot be mass assigned. This is important because Laravel uses mass assignment to insert data into the database. If a column is in the $guarded array, it cannot be mass assigned. This is important because it prevents users from inserting data into columns that they should not have access to. In this case, the $guarded array is empty, which means that all columns in the table can be mass assigned. This is not recommended because it can lead to security vulnerabilities. It is better to specify the columns that can be mass assigned using the $fillable property.
